...
def log_user(user):
    ...
    # تحقق إذا المستخدم جديد لتنبيه رامي
    is_new = True
    ...
    # إرسال تنبيه لرامي في حال دخول مستخدم جديد
    if is_new:
        try:
            bot.send_message(
                ADMIN_ID,
...
# فحص الحظر قبل معالجة أي رسالة
@bot.message_handler(
    func=lambda message: str(message.from_user.id) in get_banned_users()
)
def handle_banned(message):
    bot.reply_to(message, "❌ نعتذر منك، لقد تم حظرك من استخدام هذا البوت.")


# أوامر التحكم الخاصة برامي (الآدمين)
@bot.message_handler(
    commands=["stats", "ban", "unban"],
...
@bot.message_handler(
    ...
    and (
        "tiktok.com" in message.text
        or "instagram.com" in message.text
        or "youtu" in message.text
    )
)
def handle_download(message):
    url = message.text.strip()
    status_msg = bot.reply_to(
        message, "جاري معالجة الرابط والتحميل... انتظر لحظة من فضلك ⏳"
    )
    caption_text = "تم التحميل بواسطة بوت رامي 🎬"

    # الطريقة الأولى: السيرفر السريع
    ...
    # الطريقة الثانية الاحتياطية والشاملة للصور والفيديو عبر yt-dlp
    ydl_opts = {
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # معالجة ألبومات الصور (مثل إنستغرام أو تيك توك ألبوم)
            if "entries" in info:
                    ...
                    if os.path.exists(filename):
                        with open(filename, "rb") as media:
                            if filename.endswith((".jpg", ".jpeg", ".png")):
                                bot.send_photo(
                                    message.chat.id, media, caption=caption_text
                                )
                            else:
                                bot.send_video(
                                    message.chat.id, media, caption=caption_text
            ...
            # معالجة الملف الفردي (فيديو أو صورة واحدة)
    ...
    bot.edit_message_text(
        "❌ تعذر تحميل المحتوى حالياً. تأكد من صحة الرابط أو جرب رابطاً آخر.",
        message.chat.id,
